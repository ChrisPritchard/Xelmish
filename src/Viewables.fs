...
let colour colour (width, height) (x, y) : Viewable = 
    fun loadedAssets _ spriteBatch -> 
        spriteBatch.Draw(loadedAssets.whiteTexture, xnaRect x y width height, xnaColor colour)

let image key colour (width, height) (x, y) : Viewable = 
    fun loadedAssets _ spriteBatch -> 
        spriteBatch.Draw(loadedAssets.textures.[key], xnaRect x y width height, xnaColor colour)
