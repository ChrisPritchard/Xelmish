...
let colour (colour: Colour) (width, height) (x, y) : Viewable = 
    fun loadedAssets _ spriteBatch -> 
        spriteBatch.Draw(loadedAssets.whiteTexture, xnaRect x y width height, colour.toXnaColor())

let image key (colour: Colour) (width, height) (x, y) : Viewable = 
    fun loadedAssets _ spriteBatch -> 
        spriteBatch.Draw(loadedAssets.textures.[key], xnaRect x y width height, colour.toXnaColor())

let text font (fontSize: float) (colour: Colour) (ox, oy) (text: string) (x, y) : Viewable =
    fun loadedAssets _ spriteBatch -> 
        ...
        spriteBatch.DrawString(font, text, position, colour.toXnaColor(), 0.f, Vector2.Zero, scale, SpriteEffects.None, 0.f)
